...
void draw(stringstream &lineStream, NodeList *node_head){
    if (TooManyArguChecking(lineStream))
        return;

    float xleft, ybottom, xright, ytop;
    if(!node_head->set_draw_coords(xleft, ybottom, xright, ytop)){
        cout<<"No resistor has been inserted into the network."<<endl;
        cout<<"Please insert resistors then run this command"<<endl;
        return;
    }
    
    int node_num = (xright - 20*2) /80;
    int res_num = (ytop - 40*2) /80;
    easygl *window_ = new easygl("Resistor display", WHITE);
    window_->set_world_coordinates (xleft, ybottom, xright, ytop);
    node_head->draw(node_num, res_num, window_);
    cout<<" Draw: control passed to graphics window."<<endl;
    window_->gl_event_loop ();
